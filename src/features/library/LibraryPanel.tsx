...
import { createCanvasCatalogRepository } from "../catalog/repository";
...
export function LibraryPanel({ onAdd }: { onAdd: (item: CatalogItem) => void }) {
  ...
  const { libraries, load } = useComponentLibraryStore();
  const awsEnabled = libraries.some((library) => library.id === "aws" && library.enabled);
  const catalog = useMemo(() => createCanvasCatalogRepository(awsEnabled), [awsEnabled]);
  const items = useMemo(() => catalog.search(query), [catalog, query]);
  const categories = useMemo(() => catalog.getCategories(), [catalog]);
    ...
    <div className="library-list">
      {categories.map((category) => {
        ...
        return <section key={category} className="library-category">
          ...
          {!isCollapsed && <div className="library-items">{categoryItems.map((item) => <button key={item.type} type="button" draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "copy"; event.dataTransfer.setData("application/orbit-node", item.type); }} onClick={() => onAdd(item)} className="library-item">{item.assetPath ? <img className="library-asset-icon" src={item.assetPath} alt="" /> : <CatalogIcon icon={item.icon} className={`tone-${item.color}`} />}<span>{item.label}</span></button>)}</div>}
        </section>;
      })}
