...
function CanvasEngine({ projectId }: { projectId: string }) {
  const { screenToFlowPosition } = useReactFlow();
  const { nodes, edges, selectedId, loading, error, load, addComponent, addImage, move, connect, updateComponent, updateVisual, removeComponent, removeVisual, select } = useCanvasStore();
  ...
  useEffect(() => {
    const deleteSelected = (event: KeyboardEvent) => { const target = event.target as HTMLElement | null; if (!selectedId || target?.matches("input, textarea, select")) return; if (event.key === "Delete" || event.key === "Backspace") { event.preventDefault(); if (selectedImage) void removeVisual(projectId, selectedId); else void removeComponent(projectId, selectedId); } };
    window.addEventListener("keydown", deleteSelected);
    return () => window.removeEventListener("keydown", deleteSelected);
  }, [projectId, removeComponent, removeVisual, selectedId, selectedImage]);
  ...
  return <AppShell activeSection="canvas" onNavigate={(section) => { window.location.hash = section === "home" ? "" : section === "canvas" ? `canvas/${encodeURIComponent(projectId)}` : section; }}>
    <div className="canvas-layout">
      ...
      <aside className="properties-panel"><header><button className="active">Properties</button><button disabled>AI Assist</button></header>{selected ? <ComponentInspector projectId={projectId} component={selected} onSave={updateComponent} onRemove={removeComponent} /> : selectedImage ? <ImageInspector projectId={projectId} image={selectedImage} onSave={updateVisual} onRemove={removeVisual} /> : <div className="canvas-empty-inspector"><strong>Selecione um elemento</strong><span>Crie um Component ou adicione uma imagem para começar.</span></div>}</aside>
    </div>
  </AppShell>;
}

function ComponentInspector({ projectId, component, onSave, onRemove }: { projectId: string; component: PersistedComponent; onSave: (projectId: string, component: PersistedComponent) => Promise<void>; onRemove: (projectId: string, componentId: string) => Promise<void> }) {
...
export default function CanvasPage({ projectId }: { projectId: string }) { return <ReactFlowProvider><CanvasEngine projectId={projectId} /></ReactFlowProvider>; }
